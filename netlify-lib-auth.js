...
class Authenticator {
    ...
    /*
  returns listener function for window.addEventListener listener
  if event listener receives  string with provider AND the origin is the same as base_url then 
  new message listener is created where listener functioner is created by authorizeCallback
  this.authWindow == created auth window
  maybe sends message back
*/
    handshakeCallback(options, cb) {
        const fn = (e) => {
            if (
                ...
                return this.authWindow.age(e.data, e.origin);
            }
        };
        return fn;
    }
    ...
    authorizeCallback(options, cb) {
        const fn = (e) => {
            ...
            if (
                e.data.indexOf(
                    'authorization:' + options.provider + ':success:'
                ) === 0
            ) {
            ...
            if (
                e.data.indexOf(
                    'authorization:' + options.provider + ':error:'
                ) === 0
            ) {
    ...
    /* authenticate(options and cb) is called by handleLogin() receives 
  adds EventListener message to window: (callhandshakeCallback() with provider and scope options and cb  for listener function)
  creates URL and opens window with window.open(URL) and window.focus()
  */
